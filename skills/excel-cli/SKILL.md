...
Read the whole workbook (path omitted defaults to workbook root):
```shell
...
- Use canonical paths that do not begin with `/`.
- Supported initial paths are `""` (empty, workbook root), `<sheet>`, `<sheet>/A1`, and `<sheet>/A1:C3`.
...
- `query` is for workbook structure, not cell-content search. In the current design it enumerates sheets only at `""` (empty string, workbook root).
